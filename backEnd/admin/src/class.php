...
class Action
{
	...
	{
		extract($_POST);
		$check = $this->db->query("SELECT * FROM employees where emp_email ='$emp_email' ")->num_rows;
		echo $check;
		if ($check > 0) {
			return 2;
			exit;
			...
			if (isset($_FILES['profile_pic']) && $_FILES['profile_pic']['tmp_name'] != '') {
				...
				$move = move_uploaded_file($_FILES['profile_pic']['tmp_name'], './assets/uploads/' . $fname);
			}
			$save = $this->db->query("INSERT INTO employees set emp_full_name='$emp_full_name', emp_gender='$emp_gender', emp_dob='$emp_dob', emp_mob='$emp_mob', emp_email='$emp_email', emp_address='$emp_address', emp_joining_date='$emp_joining_date',	emp_confirmation_date='$emp_confirmation_date', emp_leaving_date='$emp_leaving_date', emp_department_name='$emp_department_name', emp_designation='$emp_designation', hod='$hod', emp_working_time='$emp_working_time',profile_pic='$fname'");
		}
		if ($save) {
			return $save;
		}
	...
	{
		extract($_POST);
		$delete = $this->db->query("DELETE FROM employees where emp_id = " . $emp_id);
		if ($delete)
			return 1;
	}
